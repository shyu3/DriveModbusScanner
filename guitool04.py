...
runcmd=14593
target_read_address = 5
target_read_cnt = 2
target_write_address = 5
target_write_cnt = 2
# Profile_run=0
# connections=[0]
# ReadSCANer=[0]
...
def str_to_num(abc):
    ...
    for i in a:
        num = (num<<8)+i
    return num
def MB_Client_READ_Address(sock,ind,_flag):
    TI=1
    startaddr=5
    quantityaddr=4
    bytecount=8
...
def MB_Client_FC23_READ_WRITE(sock,ind,_flag):
    TI=1
    startaddr=5
    quantityaddr=4
    bytecount=8
    ...
    ReceiveCnt_READ=0
    ReceiveCnt_WRITE=0
    write_date1=[10, 14]+[num for num in range(5,19)]
    write_date2=[10, 14]+[num for num in range(5,7)]+[num for num in range(784,796)]
    global runcmd
    while Kill_flag==0:
        if TI >= 65534:
            TI=0
        ...
        Byte_cnt_write=16*2
        modbus_size=43
        packet1=pack(">HHBBBBHHHHB2H14H",9,0,0,modbus_size,1,FD,addr_read,cnt_read,addr_write,cnt_write,Byte_cnt_write,*write_date1)
        # print("Request_READ :",packet)
        try:
            sock.send(packet1)
        except:
        ...
        try:
            ...
            connection_infoTABLE[ind][0]=ReceiveCnt_READ
            # connection_infoTABLE[ind][2]=receivemesg[12:].hex()
            connection_infoTABLE[ind][2]=receivemesg[12]
        ...
        packet2=pack(">HHBBBBHHHHB16H",9,0,0,modbus_size,1,FD,addr_read,cnt_read,addr_write,cnt_write,Byte_cnt_write,*write_date2)
        # print("Request_WRITE :",packet)
        try:
            sock.send(packet2)
        except:
...
def TRY_connect():
    ...
    btnfC_23.configure(state='normal')
...
def TRY_Start_Reust():
    ...
    btnfC_23.configure(state='disabled')
    CLIENT_NUM=int(entry2bt1.get())
    index=0
    global ReadSCANer
    global WriteSCANer
...
def TRY_Start_FC23_Reust():
    listBOX.insert(END," start request FD : 23")
    ...
    while index < CLIENT_NUM:
        try:
            ReadSCANer[index] = threading.Thread(target=MB_Client_FC23_READ_WRITE,args=(connections[index],index,Kill_flag))
            # WriteSCANer[index]=threading.Thread(target=MB_Client_WRITE,args=(connections[index],index,Kill_flag))
            ReadSCANer[index].daemon = True
            # WriteSCANer[index].daemon = True
            ReadSCANer[index].start()
...
btnfC_23 = Button(root,text="Use fC_23(read/write)",command =TRY_Start_FC23_Reust,state='disabled')
btnfC_23.place(x=Button_X-150,y=50+50-20,width=120,height=25)
#Start Req button textrue
...
btn5.place(x=Button_X,y=50+50+50+130-80,width=65,height=25)
#log text
frame=Frame(root)
frame.place(x=BOX_X-125,y=50+50+50+100+130,width=290,height=300-20)
...
# txt4log.insert(END,"")
# access Address
ReadAddress =Label(root, text ="ReadAddress: ")
ReadAddress.place(x=BOX_X-100,y=50+50+50+130-80+30)
ReadAddressbox=Entry(root)
ReadAddressbox.place(x=BOX_X+0,y=50+50+50+130-80+30,width=30,height=25)
ReadAddressbox.insert(END,"5913")
ReadAddressbox=Entry(root)
ReadAddressbox.place(x=BOX_X+0+50,y=50+50+50+130-80+30,width=30,height=25)
ReadAddressbox.insert(END,"0")
# tree table
